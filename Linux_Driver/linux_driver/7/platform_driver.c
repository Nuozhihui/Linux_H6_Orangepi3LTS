...
void timer_callbackfunction(unsigned long arg){
    ...
    wake_up_interruptible(&embededplatform_dev_er.queue_head);
}
...
static int embeded_platform_open (struct inode *node, struct file *filp){
    ...
    embededplatform_dev_er.timer.expires= jiffies + msecs_to_jiffies(10000);/* 超时时间 10 秒 */
...
static ssize_t embeded_platform_write (struct file *filp, const char __user *buf, size_t count, loff_t *off){	
    printk("embeded_platform_write start\n");
    wait_event_interruptible(embededplatform_dev_er.queue_head,atomic_read(&embededplatform_dev_er.keyvalue));
    printk("embeded_platform_write end\n");
    return 0;
}
