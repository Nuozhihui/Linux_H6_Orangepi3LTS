...
static ssize_t embeded_platform_write (struct file *filp, const char __user *buf, size_t count, loff_t *off){	
    ...
    /* 获取互斥体,可以被信号打断 */
    if (mutex_lock_interruptible(&embededplatform_dev_er.lock)) {
 		return -ERESTARTSYS;
 	}
	#if 0
 		mutex_lock(&embededplatform_dev_er.lock); /* 不能被信号打断 */
    #endif
...
static int embeded_platform_probe(struct platform_device *pdev)
{
    int ret = 0;
    /* 初始化互斥体 */
 	mutex_init(&embededplatform_dev_er.lock);
